...
**Keys ausentes** (renderizan `Translation missing: en.ledsc4.X`):
...
**Traducciones rotas (artefactos de machine translation)**:

- `ledsc4.solicitudes.table.th_uds = "You (formal, plural)"` — debería ser "Units" o "Qty". Es traducción automática de "Uds." (abreviatura de unidades) interpretada como pronombre "ustedes". Bug visible en la tabla de la solicitud.
...
**Lo que sí está correcto**: el bloque `ledsc4.account.*` completo (dashboard, revision, rechazada) está presente al final de `en.json` y traducido razonablemente. EN está en buen estado funcional; los gaps son cosméticos o de claves individuales nuevas que no se propagaron.

### `fr.json` — gaps y errores

FR está en estado peor que EN: tiene áreas funcionales rotas, no solo cosméticas.

**Bloques completos ausentes**:

- `ledsc4.account.dashboard.*`, `ledsc4.account.revision.*`, `ledsc4.account.rechazada.*` — **TODOS los bloques de cuenta cliente sin traducir**. Cualquier cliente francés que llegue a su área privada ve `Translation missing` en heading, cards, leads, mensajes de revisión y rechazo. **Bloqueante para clientes FR aprobados o rechazados.**
- `ledsc4.common.footer.copyright_html` — falta. El footer legal del portal en FR muestra `Translation missing` en lugar del copyright completo.
...
- `ledsc4.product.specs.cri` — ausente en FR (presente en ES con valor "CRI").
- `ledsc4.acceso.form.label_email` — falta (mismo gap que EN).
...
- `ledsc4.product.specs.ik = "OK"` — IK traducido como "OK". Debe quedarse como "IK". Probablemente artefacto de auto-traducción interpretando "IK" como afirmación.
- `ledsc4.product.specs.material = "Matériel"` — debería ser "Matériau". "Matériel" significa "equipo/hardware", no "material físico".
- `ledsc4.product.specs.regulacion = "Réglementation"` — debería ser "Variation" o "Gradation". El ES "regulación" en este contexto es dimming (regulación de intensidad), no normativa.
- `ledsc4.solicitudes.table.th_uds = "Vous."` — mismo patrón que en EN: traducción automática de "Uds." como "Vous" (ustedes).
...
**Patrón sospechoso**: el bug `th_uds` ("Uds." → "You"/"Vous.") sugiere que la traducción FR se generó desde EN en vez de desde ES. Verificable comparando claves donde ES tiene precisión que EN pierde (acrónimos técnicos, abreviaturas) y FR repite la pérdida de EN. Recomendación: auditoría dedicada a esto antes de tocar más copy FR.

### Resumen ejecutivo

| Categoría | EN | FR |
| --- | ---: | ---: |
| Bloques `ledsc4.*` completos ausentes | 0 | 3 (account.dashboard, account.revision, account.rechazada) |
| Strings vacíos en pantallas visibles | 2 | 5 |
| Keys faltantes individuales | 11 | 3 |
| Traducciones rotas con impacto al cliente | 2 (th_uds, cri) | 6 (ip, ik, material, regulacion, th_uds, registro inconsistente) |
| Copyright footer | OK | Ausente |
| Severidad global | Cosmética | Funcional |

Lo más urgente operativamente: completar el bloque `ledsc4.account.*` en FR (afecta a cualquier cliente FR aprobado o rechazado), y rellenar `portal.*` y `checkout.disclaimers.*` en FR (afecta a UX y posiblemente cumplimiento legal).
...
- **Rellenar strings vacíos críticos en `fr.json`** — `portal.heading_html`, `portal.lead`, `portal.f1_title`, `portal.f1_desc`, `checkout.disclaimers.body_html`, `common.footer.copyright_html`. El de checkout tiene implicación legal (avisos de IVA y devoluciones no se muestran al cliente FR).

- **Corregir traducciones rotas heredadas de Translate & Adapt** — `th_uds = "You (formal, plural)"` y `th_uds = "Vous."`, `cri = "ICC"`, `material = "Matériel"`, `ip = "PI"`, `ik = "OK"`, `regulacion = "Réglementation"`. Todas son artefactos de traducción automática literal sobre acrónimos o abreviaturas técnicas. Pasar por revisión humana del bloque `ledsc4.product.specs.*` y `ledsc4.solicitudes.table.*` en EN y FR.

- **Auditar origen del bug `th_uds`** — investigar si la traducción FR se hizo desde EN en vez de ES. Si es así, revisar todas las traducciones FR donde ES y EN difieren para detectar más bugs propagados (regulacion → "Réglementation" en lugar de "Variation"/"Gradation" es otra candidata del mismo patrón).
