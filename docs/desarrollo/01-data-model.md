...
| `aprobado` | Revisado OK. Acceso B2B activo. Normalmente implica Company creada — **excepto** para `instalador` (ver abajo). | edge `approve-customer` (manual desde backoffice), edge `promote-whitelist-matches` (auto desde `pg_cron`), o Shopify Flow W1 (auto-aprobación de distribuidor por whitelist, o auto-aprobación de instalador — ver `flows/W1-walkthrough.md`). |
...
| `instalador` | **Fase 2 (2026-07)**. Co-ocurre siempre con `aprobado`. Marca un cliente auto-aprobado **sin Company**. El discriminador de carril es `b2b.sector === "instalador"` (fijo por la landing dedicada `/pages/acceso-instalador`), comprobado en Flow W1 **antes** de la lógica de whitelist — la whitelist nunca decide este tag, solo actúa dentro del carril de distribuidor (sector ≠ `instalador`). También activa el markup visual +15% de precios (`window.LEDSC4_INSTALLER_MARKUP` en `layout/theme.liquid`, Fase 1). Ortogonal a `{pendiente,aprobado,rechazado}` pero en la práctica solo aparece junto a `aprobado`. | Shopify Flow W1 (ver `flows/W1-walkthrough.md`). |
...
|---|---|---|---|---|---|
| `empresa` | `single_line_text_field` | ✅ | Sí | edge `register-b2b-customer` | Razón social. **Obligatorio salvo `sector === "instalador"`** (Fase 2, 2026-07): la landing `/pages/acceso-instalador` no tiene este campo. La edge fuerza el valor a vacío para ese sector (aunque el body lo traiga relleno) y omite el metafield entero cuando queda vacío (Shopify rechaza `single_line_text_field` con value ""). **Es el discriminador que usa `create-company-for-customer` para no crear Company** — no confundir con `sector`, que es el discriminador que usa Flow W1 para decidir el rol. Obligatorio siempre en `complete-b2b-registration` (ese carril es siempre distribuidor). |
| `nif` | `single_line_text_field` | ✅ | Sí | edge `register-b2b-customer` | NIF/CIF/NIE validado con dígito de control en el form y reforzado por Flow W1. **Opcional si `sector === "instalador"`** (Fase 2); si se rellena, igual se valida formato. Obligatorio siempre en `complete-b2b-registration`. |
